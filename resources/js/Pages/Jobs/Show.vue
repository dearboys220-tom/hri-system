...
const apply = () => {
    applyForm.post(`/jobs/${props.job.id}/apply`);
};
...
const daysLeft = computed(() => {
    ...
    const diff = Math.ceil((new Date(props.job.application_deadline) - new Date()) / 86400000);
    return diff;
});

const deadlineLabel = computed(() => {
    if (daysLeft.value === null) return '-';
    if (daysLeft.value < 0) return 'Sudah ditutup';
    if (daysLeft.value === 0) return 'Hari ini!';
            ...
            <div class="lg:col-span-2 space-y-5">
                ...
                <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
                    <div class="flex items-start gap-4">
                        <div class="w-16 h-16 rounded-xl bg-gray-100 border border-gray-200 overflow-hidden flex items-center justify-center flex-shrink-0">
                        ...
                        <div class="flex-1 min-w-0">
                            ...
                            <div class="flex flex-wrap gap-2 mt-3">
                                <span :class="['text-xs px-3 py-1 rounded-full font-medium', employmentColor(job.employment_type)]">
                                    {{ job.employment_type }}
                                </span>
                                <span v-if="categoryName" class="text-xs px-3 py-1 rounded-full bg-gray-100 text-gray-600">
                                    {{ categoryName }}
                                </span>
                                <span v-if="subcategoryName" class="text-xs px-3 py-1 rounded-full bg-gray-100 text-gray-600">
                                    {{ subcategoryName }}
                                </span>
                            </div>
                        </div>
                    ...
                    <div class="mt-5 grid grid-cols-2 sm:grid-cols-3 gap-3">
                        ...
                        <div class="bg-gray-50 rounded-xl p-3 text-center">
                            <p class="text-xs text-gray-400 mb-1">⏳ Batas Lamar</p>
                ...
                <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
                    ...
                    <div class="space-y-3 text-sm">
                        <div v-if="job.working_hours" class="flex justify-between border-b border-gray-50 pb-2">
                            <span class="text-gray-500">Jam Kerja</span>
                            <span class="text-gray-800 font-medium">{{ job.working_hours }}</span>
                        </div>
                        <div v-if="job.working_days?.length" class="flex justify-between border-b border-gray-50 pb-2">
                            <span class="text-gray-500">Hari Kerja</span>
                            <span class="text-gray-800 font-medium">{{ job.working_days.join(', ') }}</span>
                        </div>
                        <div v-if="job.language_requirements?.length" class="flex justify-between border-b border-gray-50 pb-2">
                            <span class="text-gray-500">Bahasa</span>
                            <span class="text-gray-800 font-medium">{{ job.language_requirements.join(', ') }}</span>
                        </div>
                        <div v-if="job.age_min || job.age_max" class="flex justify-between border-b border-gray-50 pb-2">
            ...
            <div class="space-y-5">
                ...
                <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-5 lg:sticky lg:top-20">
                    ...
                    <template v-if="auth?.user?.role_type === 'applicant'">
                        ...
                        <div v-if="alreadyApplied"
                            class="w-full text-center bg-green-50 border border-green-200 text-green-700 font-bold py-3 rounded-xl text-sm">
                            ✅ Sudah Melamar
                        </div>
                        ...
                        <button
                            ...
                            {{ applyForm.processing ? 'Memproses...' : '🚀 Lamar Sekarang' }}
                        </button>
                        <p class="text-xs text-center text-gray-400 mt-2">
                            Pastikan CV kamu sudah lengkap
                        </p>
                    </template>
                    ...
                    <template v-else-if="!auth?.user">
                        <Link href="/login"
                              class="block w-full text-center bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-3 rounded-xl transition text-sm">
                            Login untuk Melamar
                        </Link>
                        <p class="text-xs text-center text-gray-400 mt-2">
                            Belum punya akun? <Link href="/login" class="text-indigo-600 underline">Daftar gratis</Link>
                        </p>
                    </template>
                ...
                <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-5">
                    ...
                    <div class="flex items-center gap-3 mb-3">
                        <div style="width:48px; height:48px; min-width:48px; overflow:hidden; border-radius:8px; border:1px solid #c7d2fe; background:#eef2ff; display:flex; align-items:center; justify-content:center;">
                            <img v-if="company?.company_logo"
                                :src="`/storage/${company.company_logo}`"
