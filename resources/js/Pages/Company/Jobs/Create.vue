...
const categoryOptions = [
    'HR & Rekrutmen', 'Keuangan & Akuntansi', 'Teknologi Informasi',
    'Pemasaran & Sales', 'Operasional', 'Hukum & Kepatuhan',
    'Administrasi', 'Logistik', 'Lainnya',
];
        ...
        <div class="py-8">
            <div class="mx-auto max-w-3xl px-4 sm:px-6 lg:px-8 space-y-5">
                ...
                <div v-if="!isVerified" class="bg-red-50 border border-red-200 rounded-2xl p-6 text-center">
                    ...
                    <Link href="/company/dashboard" class="inline-block mt-4 bg-red-600 text-white px-5 py-2 rounded-xl text-sm font-medium hover:bg-red-700 transition">
                ...
                <template v-else>
                    <!-- 無料/有料バナー -->
                    <div v-if="isFreePostAvailable" class="bg-gradient-to-r from-emerald-500 to-teal-500 rounded-2xl p-4 text-white flex items-center gap-3">
                    ...
                    <form @submit.prevent="submit" class="space-y-5">
                        ...
                        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 space-y-4">
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                ...
                                <div>
                                    ...
                                    <p v-if="form.errors.employment_type" class="text-red-500 text-xs mt-1">{{ form.errors.employment_type }}</p>
                                </div>
                            </div>
                            ...
                            <div v-if="form.category && categories[form.category]">
                                ...
                                <select v-model="form.subcategory"
                                    ...
                                    <option v-for="(label, subkey) in categories[form.category].subcategories" :key="subkey" :value="subkey">
                                        {{ label }}
                                    </option>
                                </select>
                            </div>
                            ...
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">Lokasi Penempatan Kerja <span class="text-red-500">*</span></label>
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <input v-model="form.application_deadline" type="date"
                                        class="w-full border border-gray-300 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-400" />
                                    <p v-if="form.errors.application_deadline" class="text-red-500 text-xs mt-1">{{ form.errors.application_deadline }}</p>
                                </div>
                                <div>
                        ...
                        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 space-y-4">
                            ...
                            <div>
                                ...
                                <p v-if="form.errors.job_description" class="text-red-500 text-xs mt-1">{{ form.errors.job_description }}</p>
                            </div>
                            ...
                            <div>
                                ...
                                <textarea v-model="form.special_requirements" rows="3"
                                    class="w-full border border-gray-300 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-400"
                                    placeholder="Contoh: Bersedia dinas luar kota, memiliki kendaraan pribadi..."></textarea>
                            </div>
                        </div>
                        ...
                        <div class="flex justify-end gap-3 pb-8">
                            ...
                            <button type="submit" :disabled="form.processing"
                                ...
                                {{ form.processing ? 'Memproses...' : (isFreePostAvailable ? '🎁 Posting GRATIS' : '💰 Posting Lowongan') }}
                            </button>
                        </div>
